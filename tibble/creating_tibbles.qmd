...
__Note:__ The `iris` dataset is built-in to base R. You can see all the pre-loaded datasets in R with `data()`

## File to tibble

We can use `readr`'s function `read_csv()` to read in a file as a __tibble__.

More information: [`readr` page](../readr/readr.qmd) 

Ensure you have the following file downloaded: [all_plants_details.csv download](https://neof-workshops.github.io/Tidyverse/data/all_plant_details.csv)
