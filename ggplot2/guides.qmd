...
- `legend_guide()`: Change the position of the title and change the number of rows/columns the values are placed within.
- `guide_axis()`: Allow the axis values to dodge each other or change their angle.

When using these functions we place them within the `guide()` __function/component__ so we can set the __aesthetic__ we specifically want to change.
...
This is useful when you have a large amount of values to display or setting the legend below the plot.
...
- Set all the text to 20 ([Set all text size with `theme()`](/ggplot2/theme.qmd#all-text))
- Set the x axis text to dodge with `guide(x = guide_axis(n.dodge = 2))`. The 2 represents the number of rows/columns used for dodging.
