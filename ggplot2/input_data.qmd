...
When using  `tidyverse` packages we are primarily working with __tidy/long__ data.
This is by design.
...
For example, you may have a scatterplot comparing 2 continuous measures from 2 different columns (e.g. height vs weight).
...
In this case there seems to be no linear correlation but that is because we randomly created the dataset in a very naive manner.
