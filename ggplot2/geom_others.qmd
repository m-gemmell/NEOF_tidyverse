...
We'll also make a randomly subsampled dataset for plots that are better suited to less data.
...
`r hide("View mushroom_sample_tbl contents")`
```{r, echo=FALSE}
library("reactable")
options(reactable.theme = reactableTheme(
  color = "hsl(233, 9%, 87%)",
...
The function `ggplot2::geom_hex()` requires the `hexbin` package to be installed.
...
A rug plot is an addition to a scatter plot.
