...
We'll __facet__ the data by the __variable__ `class`, __facetting__ by columns with `dplyr::facet_grid(cols = dplyr::vars(class))`.
...
We'll __facet__ the data by the __variable__ `cap_color`, __facetting__ by rows with `ggplot2::facet_grid(rows = dplyr::vars(cap_color))`.
...
The function `ggplot2::facet_wrap()` will wrap __panels__ over multiple rows and/or columns.
...
It is important to note that with this method the scales are fixed (i.e. all the __panels__ have the same scale).
To learn more and edit this see the [facet scales section](#facet-scales).
...
__Facets__ become especially useful when you have a lot of __variables__ you would like to represent as aesthetics.
...
mushroom_tbl |>
    ggplot2::ggplot(aes(x=stem_width,y=stem_height,color=cap_shape,shape=class)) +
        ...
        #Facet grid class by columns and cap_color by rows
        ggplot2::facet_grid(
            cols = dplyr::vars(gill_attachment), rows = dplyr::vars(gill_color)) +
        #Labels
        ggplot2::labs(
            title="Scatter plot showing features of mushrooms",
            subtitle="Facetted by gill colour (rows) and gill attachment (columns)",
