...
The `ggplot2::theme()` component can be used to alter the appearance of a ggplot.
...
Although you can set the plot size with [`ggplot2::ggsave()`](/ggplot2/ggsave.qmd#height-and-width-choice) to try to get appropriate text sizes this is not always the optimal way.
Instead the text/font size can be chosen in the `ggplot2::theme()` function.

### All text

Create the [default box plot](/ggplot2/geom_boxplot.qmd#default-box-plot).
We will set the the text size with the `ggplot2::theme()` option `text=`.

This options then needs to be provided with the function `ggplot2::element_text()` which itself has different options.
These include:
