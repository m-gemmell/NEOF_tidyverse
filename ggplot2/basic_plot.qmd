...
__Data__ for `ggplot()` needs to be [long formatted](http://localhost:7619/tidyr/pivot_longer.html) so each column can be assigned to an __aesthetic__.
...
More information: [Ggplot object](/ggplot2/ggplot_object.qmd)
...
More information: [Layers](/ggplot2/)
...
We do not need to provide `ggplot2::ggplot()` with `data=` and `mapping=` as long as we provide the __data first__ and the __mapping second__.
