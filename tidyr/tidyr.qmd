...
There are a lot more `tidyr` functions than those covered here.
...
- [__`pivot_longer()`__](/tidyr/pivot_longer.qmd): Longifies data, transforming it from wide to long
  - Many `Tidyverse` packages are built with long data in mind, especially [`ggplot2`](/ggplot2/ggplot2.qmd)
- [__`pivot_wider()`__](/tidyr/pivot_wider.qmd): Widens data, transforming it from long to wide
...
- [__`separate_wider_delim()`__](/tidyr/separate_wider.qmd): Splits a string column into multiple columns by a delimiter
- [__`unite()`__](/tidyr/unite.qmd): Combines/unites multiple string columns into one column
...
- [__`drop_na()`__](/tidyr/drop_na.qmd): Remove rows with NAs
- [__`replace_na()`__](/tidyr/replace_na.qmd): Replace NA values