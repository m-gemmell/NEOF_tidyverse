...
You can remove rows with `NA`s in various ways with the `drop_na()` function.

- Remove rows that contain at least one `NA` value
- Remove rows with `NA`s in one specific column
- Remove rows with `NA`s in multiple specific columns
...
## NAs in multiple specific columns
...
__Note:__ There is an `NA` in the 10th row of the `life_span` column as we did not specify it with `drop_na()`.
