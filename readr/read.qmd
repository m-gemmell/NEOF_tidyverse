...
- `readr::read_csv2()`: For some European fromatted data.
    - Same as `readr::read_delim()` but uses semi colons (`;`) as the default delimiter and uses commas (`,`) for decimal points
...
This can be useful if values in the data contain the three common delimiters (comma, tab, and space).
