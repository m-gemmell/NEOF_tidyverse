...
- `id=`: Adds an ID column containing the file path
    - Useful when reading in multiple files and combining the data into one __tibble__
- `skip=`: Skips a specified number of lines from the top of the file
    - Useful if the top of the file contains information (e.g. annotation) on the data that you don't want included in the __tibble__
- `n_max=`: Specify the max number of lines/rows to read into the __tibble__
    - Useful for only getting a small amount of the data for creating/testing/debugging code
