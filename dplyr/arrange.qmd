...
`dplyr::arrange()` will arrange the rows by the selected column in ascending fashion (lowest to highest/a->z). You can use the `desc()` function on columns you wan't to be ordered in descending fashion (highest to lowest/z->a).
