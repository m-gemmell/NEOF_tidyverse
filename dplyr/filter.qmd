...
The `dplyr::filter()` function can be used to extract rows that meet specified conditions.
...
Filter the __tibble__ to only contain rows/observations from the `rec_year` 2000 and onwards (>2000).
...
Filter to contain observations where `animal_code` is B-301, A1, or W-008.
...
You can use [`grepl()`](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/grep) to filter rows that contain a string in a column.

Extract rows where `animal_code` column/variable contains "A".

```{r}
knz_bison |>
    dplyr::filter(grepl("A", animal_code))
```

Extract rows where `animal_code` does not contain "B".

```{r}
knz_bison |>
    dplyr::filter(!grepl("B", animal_code))
```

You can use **regular expressions**.

Extract rows where `animal_code` column/variable starts with "A".
...
Extract rows where `animal_code` either begins with "8" or ends with "9".

```{r}
knz_bison |>
    dplyr::filter(grepl("^8|9$", animal_code))
```
...
1. Extract females
2. Extract observations with `animal_weight` between 900 and 1000
3. Extract samples from the `rec_year` 2000
...
For removal of NAs please see the [`tidyr` `drop_na()` page](/tidyr/drop_na.qmd).