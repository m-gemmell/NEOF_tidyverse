...
The preferred method when using `dplyr` and other `tidyverse` package functions is to use __pipes__.

The __tidyverse__ pipe symbol is `|>` (historically it was `%>%`).

__Keyboard shortcut for `|>`:__ 
...
maples_tbl <- lterdatasampler::hbr_maples
maples_tbl
```
...
To assign the output of a piped function you can utilise the assignment operator as usual.
...
::: {.callout-note}
Pipes are not only limited to `tidyverse` functions.
:::
...
The below code carries out the same steps but in a less visually clear method with nested functions.
