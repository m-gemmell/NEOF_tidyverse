...
You can create a vector containing the column/variable names you want to select.
This requires the use of the [`all_of()`](https://tidyselect.r-lib.org/reference/all_of.html) function.

```{r}
#Vector of column names
vars <- c("species", "brain_wt", "total_sleep", "life_span")
#Select columns from vector
mammal_sleep_tbl |> dplyr::select(all_of(vars))
```

If you include variables/column names that do not exist in the __tibble__ then an error will occur and the command will not run.

## Any of

The [`any_of()`](https://tidyselect.r-lib.org/reference/all_of.html) function won't check if a variable exists or not.
This means you can include variables/column names that don't exist and the function will work with no error.

```{r}
#Vector of column names
vars <- c("genus", "species", "brain_wt", "total_sleep", 
            "total_awake", "life_span", "fake_123")
#Select columns from vector
mammal_sleep_tbl |> dplyr::select(any_of(vars))
```

This is especially useful to remove variables as if you try to remove a variable after already removing it the function will still work.
To unselect variables place a `-` in front of the `any_of()` function.

```{r}
#Vector of column names
vars <- c("genus", "species", "brain_wt", 
            "total_sleep", "total_awake", "life_span", "fake_123")
#Unselect columns from vector
mammal_sleep_tbl |> dplyr::select(-any_of(vars))
```
...
Select the last column with [`last_col()`](https://tidyselect.r-lib.org/reference/everything.html) helper function.
...
Select columns using a prefix of the column names to extract with the [`starts_with()`](https://tidyselect.r-lib.org/reference/starts_with.html) helper function.
...
Select columns using a prefix of the column names to extract with the [`ends_with()`](https://tidyselect.r-lib.org/reference/starts_with.html) helper function.
...
Select columns using a string the column names will contain with the [`contains()`](https://tidyselect.r-lib.org/reference/starts_with.html) helper function.
...
The [`matches()`](https://tidyselect.r-lib.org/reference/starts_with.html) function is similar to the above [`contains()`](https://tidyselect.r-lib.org/reference/starts_with.html) function but it can be used to select with regular expressions.
